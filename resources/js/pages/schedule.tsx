...
import { dashboard, schedule, schedule_delete, schedule_save } from '@/routes';
import { type BreadcrumbItem, type Schedule, type User } from '@/types';
...
const breadcrumbs: BreadcrumbItem[] = [
    { title: 'Налаштування розкладу', href: dashboard().url },
];
...
function RangeDay({
    ...
    return (
        <PickersDay
            {...other}
            day={day}
            selected={false}          // disable MUI default ring
            onClick={() => onSelect?.(day)}
            sx={{
                // range edge (start / end)
                ...(isEdge && {
                ...
                // working days (already have schedule) — shown on top of range
                ...(isWorking && !isEdge && {
                    backgroundColor: '#2bc046',
                    border:          '1px solid #000',
                    borderRadius:    '50%',
                }),
                // working AND edge — keep edge color, add bottom dot instead
                ...(isWorking && isEdge && {
...
// ─── Page ─────────────────────────────────────────────────────────────────────
export default function ScheduleList({ masters, schedules, masterSelected }: Props) {
    const [selectedMaster, setSelectedMaster] = useState<User>(masterSelected);
    ...
    // ── prefill hours from existing schedule for a given day ─────────────────
    ...
    const handleSelect = (day: Dayjs) => {
        if (phase === 'start' || !rangeStart) {
            ...
            if (day.isBefore(rangeStart, 'day')) {
                // clicked earlier than start → make it the new start
                setRangeStart(day);
                setRangeEnd(null);
                prefillFromDay(day);
                // phase stays 'end'
            } else if (day.isSame(rangeStart, 'day')) {
                // same day → single-day selection, reset phase
                setRangeEnd(null);
                setPhase('start');
            } else {
                // clicked later → set end, complete the range
                setRangeEnd(day);
                setPhase('start');   // ready for next new selection
            }
        }
    };

    // ── master change ─────────────────────────────────────────────────────────
    const handleMasterChange = (_: unknown, user: User | null) => {
        if (!user) return;
        ...
        router.get(
            schedule(),
            { selectedMasterId: user.id },
            { preserveState: true, replace: true, only: ['schedules', 'masterSelected'] },
        );
    };
    ...
    const handleDelete = () => {
        ...
        router.delete(schedule_delete().url, {
            data: {
                user_id:   selectedMaster.id,
                date_from: rangeStart.format('YYYY-MM-DD'),
                date_to:   isRange ? rangeEnd!.format('YYYY-MM-DD') : undefined,
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Налаштування розкладу" />
                ...
                <div className="grid grid-cols-2 gap-10 rounded-xl border border-sidebar-border/70 p-5 dark:border-sidebar-border">
                    ...
                    <div>
                        <Autocomplete
                            ...
                            renderInput={(params) => (
                                <TextField {...params} label="Виберіть майстра" />
                            )}
                        />
                        ...
                        <LocalizationProvider dateAdapter={AdapterDayjs}>
                            <DateCalendar
                                value={null}
                                onChange={() => {}}   // handled via slotProps onSelect
